...
namespace rtdenoise {
    ...
    template<typename T> 
    void upscale_attn_bwd_cpu_pixel(
        const int C, 
        const int HU,
        const int WU,
        const int HD,
        ...
        for(int i = yd - hks; i <= yd + hks; i++) {
            for(int j = xd - hks; j <= xd + hks; j++) {
                ...
                for(int c = 0; c < C; c++) {
                    ...
                    if(is_inside) {
                        dLdk[c * HD * WD + i * WD + j] += dLdlogit * q[c * HU * WU + y * WU + x];
                    }
                ...
                if(is_inside) {
                    ...
                    for(int k = 0; k < 3; k++) {
                        dLdv[k * HD * WD + i * WD + j] += weight * local_dLda[k];
                    }
                }
    ...
    template<typename T>
    void upscale_attn_bwd_cpu_impl(
        const int N,
        const int C, 
        const int HU,
        const int WU,
    ...
    at::Tensor upscale_attn_bwd_cpu(
        at::Tensor q,
        at::Tensor k,
        at::Tensor v,
        at::Tensor b,
        ...
        TORCH_INTERNAL_ASSERT(o.device().type() == at::DeviceType::CPU);
        TORCH_INTERNAL_ASSERT(L.device().type() == at::DeviceType::CPU);
        TORCH_INTERNAL_ASSERT(m.device().type() == at::DeviceType::CPU);
        ...
        upscale_attn_bwd_cpu_impl<float>(
            q.size(0),
            q.size(1),
            q.size(2),
            q.size(3),
    ...
    TORCH_LIBRARY_IMPL(rtdenoise, CPU, m) {
        ...
        m.impl("upscale_attn_bwd", &upscale_attn_bwd_cpu);
    }
